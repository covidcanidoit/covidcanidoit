  ...
  <div>
    ...
    <div class="edit-form">
      <div class="form-group">
        <label>Score</label>
        <input @input="saveField('riskScore', $event)" class="form-control" type="text" :value="this.riskLevel.riskScore" />
      </div>
      <div class="form-group">
        <label>Name</label>
        <input
...
export default {
  ...
  methods: {
    saveField(name, event) {
      ...
      db.ref('content/riskLevels').child(this.riskScore - 1).child(name).set(event.target.value);
    }
  }
};
</script>
